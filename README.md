...
Self-hosted local desktop agent. The model is **Qwen3.5-27B** running on this computer through llama.cpp (or any OpenAI-compatible server you point at). The web portal at [http://127.0.0.1:4780](http://127.0.0.1:4780) is the control surface. Command has a local Speak button (Whisper when installed; Windows SAPI/espeak for spoken results). Use **Guide & Workflows** for operating instructions and one-click templates. Optional Browser Use and OpenHands workers are adapters — Playwright and native coding tools remain the default.
