...
Tag-only hold for full AI-assistant / harness repos (hermes-agent, openhuman, F.R.I.D.A.Y, deer-flow, openclaude, opencode, locally-uncensored). **No butler / voice / system-prompt merge.** First-party session presence and voice is [RFC-0137](docs/rfcs/0137-persona-presence-shape-and-voice-binding.md); it does not promote this hold. Download ≠ integrate. Personality track is a later named RFC after Taco promotes it. **RFC:** [`docs/rfcs/0104-persona-candidates-pack.md`](docs/rfcs/0104-persona-candidates-pack.md). **Status:** accepted hold (not an implement ticket now). **Local clone:** `/workspace/projects/persona/<name>` · `C:\Users\daanv\projects\jarvis-ig\persona\<name>`. **Lane:** later (Architect / UX catalog badges only until promoted).
