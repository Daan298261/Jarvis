...
def metrics_from_task(task: Task, tool_calls: list[ToolCallRecord] | None = None) -> dict[str, Any]:
    ...
    return {
        ...
        "model_time_seconds": round(float(task.model_ms or 0) / 1000.0, 3),
        "tool_time_seconds": tool_time,
        "model_calls": int(task.model_calls or 0),
        "tool_calls": int(task.tool_call_count or len(calls)),
        "retries": int(task.retries or 0),
        "schema_errors": max(schema_errors, int(task.schema_errors or 0)),
