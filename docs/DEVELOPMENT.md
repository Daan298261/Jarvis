...
**Process first:** read [PROCESS.md](PROCESS.md). Implement **one** named RFC or **one** Development Queue item per session; pull latest `development`, branch from it, and open one PR against `development`. CoS/PR fixer land onto `development`; `main` stays stable until CoS or Taco call a release cut. Product intent and the queue live in [`JARVIS_MASTER_PLAN.md`](../JARVIS_MASTER_PLAN.md) §57–58 — update only the lines that match your shipped ticket.
