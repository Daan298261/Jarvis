...
| Role | Does | Does not |
| --- | --- | --- |
| **ChatGPT / Codex (design)** | Write RFCs under [`docs/rfcs/`](rfcs/) | Rewrite `JARVIS_MASTER_PLAN.md` wholesale; paste megabyte spec updates into the master plan |
| **Cursor cloud worker (implement)** | Implement **one** named RFC or **one** named Development Queue item | "Continue all development"; merge unrelated PRs; re-audit the repo; rewrite §57 Current State from scratch |
...
4. Optionally add or refine **one** matching line in `JARVIS_MASTER_PLAN.md` §58 Development Queue.
5. After merge, set RFC status to `implemented` and tick the queue item.
...
> Add an RFC under `docs/rfcs/` for [one concern]. Use `TEMPLATE.md`. Do not edit `JARVIS_MASTER_PLAN.md` except one optional queue line in §58.

**Cursor worker — implement**

> Implement `docs/rfcs/NNNN-slug.md` only. Branch from `cursor/local-qwen-desktop-agent` as `cursor/<slug>-99ea`. Run pytest. Update only matching §57–58 lines. Open PR against `cursor/local-qwen-desktop-agent`. Do not merge other PRs. Composer 2.5 standard model.

**Desktop sign-off (human)**

> On Windows with GGUFs loaded: run `python tests\run_e2e.py`. If pass, update queue item [name] to VERIFIED in §58.
