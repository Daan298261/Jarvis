...
- [x] Thought / process UI default collapsed behind a chevron on owner chat (Classic + HUD) — not in #188; RFC-0067 hide-chrome + Show work (`jarvis.chat.showWork` default off) remains the owner-chat contract; chevron polish is UX follow-up
...
Landed on `development` via #188 @ `99e1028` (backend speak path: social/technical class, speak filter, natural prose, early TTS). Thought-process chevron was not in #188; RFC-0067 hide-chrome + Show work remains the owner-chat contract, chevron polish is UX follow-up. Prior ledger tick #190 closed without merge.
