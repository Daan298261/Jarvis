...
**Status:** implemented — **availability assumptions updated by [RFC-0171](0171-system-one-reflex-lane-jev-laya-priority.md)** (2026-09-24)  
...
> **RFC-0171 update (2026-09-24):** TypeSafe Jev is now **publicly usable**, not waitlist-only. Settings / status copy must not imply early-access waitlist as the primary gate. **Still required:** explicit owner cloud opt-in (`jev_optional` / entitled `jev_plus`), a real TypeSafe key, and a **real probe** before any `source: "jev"` attribution. Local Reflex Lane (deterministic rules + optional managed Laya) is the default zero-cloud path. “Notify when ready” remains a harmless interest flag; it never means connected.
...
**Current reality (updated RFC-0171, 2026-09-24):** Jev is **publicly usable**. Implement still requires (1) owner cloud opt-in and (2) a real probe before attributing decisions to Jev. Local Reflex (rules + optional Laya) is the default. Settings may retain a “notify / docs” CTA, but must not treat waitlist as the primary availability state. Shipping a fake Jev client, canned decisions, or a soft-fail local LLM that pretends to be Jev is forbidden.
