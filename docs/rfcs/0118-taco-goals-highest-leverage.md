...
**Related (do not rewrite):** RFC-0108 (in-flight), RFC-0109 (amended this PR), RFC-0115, RFC-0117 tiny-front (implemented; colliding journal RFC-0117 stays accepted), RFC-0107 (accepted until desktop sign-off), RFC-0005 (implemented), RFC-0020 (accepted, thin), RFC-0110 (implemented), RFC-0114 (implemented). Thin new contracts: [RFC-0119](0119-coding-and-3d-real-tools.md), [RFC-0120](0120-projects-folder-chats-db-media-placement.md).
...
### 3 — **RFC-0119** coding + 3D modelling **real tools**

| | |
| --- | --- |
| **Maps to** | Goal 2 (tool calling for coding + 3D modelling) |
| **Reuse vs new** | **New thin RFC-0119.** RFC-0005 (`implemented`) is worktree isolation, not a coding/3D tool contract. Native `filesystem` / `terminal` / `python` / `git` / `verify_code` exist; `code_worker` is optional OpenHands. There is **no** Blender / OpenSCAD / local DCC tool. 0119 demands real edit/run/test calls and a real 3D CLI path, discovered via RFC-0107 tool search, gated by RFC-0110 approvals. |
...
### 4 — **RFC-0120** projects folder + chats-in-DB + media placement

| | |
| --- | --- |
| **Maps to** | Goal 4 (projects folder structure; chats in internal DB; media on disk colocated with DB-host, else storage node) |
| **Reuse vs new** | **New thin RFC-0120** (extends, does not replace, RFC-0020). RFC-0020 (`accepted`) is knowledge retrieval/workspaces — too thin for folder layout, chat persistence, and blob topology. Portal Projects today are **browser `localStorage`** (`frontend/src/projects.ts`); `conversations` already live in SQLite but are not project-scoped. `PORTAL_UX.md` currently says portal-local grouping and no new REST resource — 0120 supersedes that for durable projects (Architect updates `PORTAL_UX.md` after accept; this PR does not edit that file). |
...
| **Why it wins** | Survey: further front-responder architecture is already shipped; Pipecat is a later voice transport. The remaining 0107 gap is proving the bound vault and per-turn tool search actually enter owner turns (not décor). That cuts prompt junk (speed) and is how 0119 tools get pulled without stuffing the catalog. |
...
RFC-0108 continues          (in-flight; do not divert)
    ...
    → RFC-0119 implement    (coding + 3D real tools; needs 0115 + 0107 search + 0110)
    → RFC-0120 implement    (projects + chats-in-DB; media paths with/after 0109)
...
- [x] At most two new implement RFCs: 0119 (coding+3D), 0120 (projects/chats/media placement)
- [x] §59 Decision Log pointer only (no §57/§58 rewrite)
- [ ] Implementers take the **named** RFC (0109 / 0115 / 0119 / 0120 / 0107 remaining), not this memo, as the ticket
...
- Number **0118** — the two existing 0117 files (tiny-front **implemented**; durable-state journal **accepted**) are left as-is.
